...
generate --name "AWS Python Proof" --lang python --tier aws --auth none --db none
...
PYTHONPATH="$PYTHON_MCP/src" python3 -m py_compile "$PYTHON_MCP/src/server.py"

generate --name "AWS TypeScript Proof" --lang typescript --tier aws --auth none --db none
